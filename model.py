...
class Model(object):
  ...
  def __init__(self, imgs, is_training, use_skip_connections,
    ...
    with tf.variable_scope("train_test_model", reuse=tf.AUTO_REUSE) as scope:
      ...
      assert use_skip_connections is False, "--use-skip-connections not going to work with padding=SAME"
...
#      model = slim.conv2d(model, num_outputs=2*base_filter_size, kernel_size=3, scope='d2')
      model = slim.conv2d_transpose(model, num_outputs=4*base_filter_size,
                                    kernel_size=3, stride=2, padding='VALID', scope='d2')
      if use_batch_norm:
        model = slim.batch_norm(model, decay=0.9, is_training=is_training)
...
#      model = slim.conv2d(model, num_outputs=base_filter_size, kernel_size=3, scope='d3')
      model = slim.conv2d_transpose(model, num_outputs=2*base_filter_size,
                                    kernel_size=3, stride=2, padding='VALID', scope='d3')
      if use_batch_norm:
        model = slim.batch_norm(model, decay=0.9, is_training=is_training)
...
#        dump_shape_and_product_of('d3+e1', model)
      ...
      # NCS hacktastic! sigmoid on 2d image causes read error :/
      # expect to deal with endless "no output" errors ...
